...
class ZamptoAdapter(BaseAdapter):
    ...
    async def buy_product(self, product_id: str, quantity: int, idempotency_key: str) -> dict:
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                r = await client.post(f"{self.base_url}/buy", headers=self._headers(), json=payload)
    ...
    async def get_orders(self, limit: int = 50) -> list:
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if r.status_code == 200:
                    data = r.json()
                    return data if isinstance(data, list) else data.get("data", data.get("orders", []))
                return []
        except Exception:
            return []
